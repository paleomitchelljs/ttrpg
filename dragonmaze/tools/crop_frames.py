...
STRIPS = {
    ...
    # spawnee-* now come from a Nano Banana pose grid via tools/slice_grid.py
...
def main():
    ...
    # The manifest lists every PNG in assets/sprites/ (from this tool and
    # from slice_grid.py), so the two never clobber each other's SPRITES keys.
